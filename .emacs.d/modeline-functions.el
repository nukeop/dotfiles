...
(defun custom-modeline-flycheck-status ()
  (let* ((text (pcase flycheck-last-status-change
		 ...
		 (`not-checked (propertize(format " %s " (all-the-icons-faicon "times" :v-adjust 0.025))
                              'face '(:foreground "#dc322f")))
		 (`errored     (propertize(format " %s " (all-the-icons-faicon "exclamation-triangle" :v-adjust 0.025))
                              'face '(:foreground "#dc322f")))
